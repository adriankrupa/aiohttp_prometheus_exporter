...
class TestMiddleware:
    ...
    @pytest.mark.parametrize("path,path_template,server_response",
                             [('/200', '/200', 200),
                              ('/path/123', '/path/{value}', 200),
                              ('/random_path', '__not_matched__', 404)])
    async def test_ok(self, client: TestClient, path: str, path_template: str, server_response: int):
        ...
        assert_entry_exist(families, "aiohttp_requests", {"method": "GET",
                                                          "path_template": path_template,
                                                          "remote": "127.0.0.1",
                                                          "scheme": "http"}, 1.0)

        assert_entry_exist(families, "aiohttp_responses", {"method": "GET",
                                                           "path_template": path_template,
                                                           "remote": "127.0.0.1",
                                                           "scheme": "http",
                                                           "status_code": f"{server_response}"}, 1.0)

        assert_entry_exist(families, "aiohttp_requests_in_progress", {"method": "GET",
                                                                      "path_template": path_template,
                                                                      "remote": "127.0.0.1",
                                                                      "scheme": "http"}, 0.0)
        ...
        assert_entry_exist(families, "aiohttp_request_duration_seconds", {"method": "GET",
                                                                          "path_template": path_template,
                                                                          "remote": "127.0.0.1",
                                                                          "scheme": "http"})
    ...
    async def test_exception(self, client: TestClient, path: str, path_template: str, server_response: int):
        ...
        assert_entry_exist(families, "aiohttp_exceptions", {"method": "GET",
                                                            "path_template": "/exception",
                                                            "remote": "127.0.0.1",
                                                            "scheme": "http",
                                                            "exception_type": "ValueError"}, 1.0)


def assert_entry_exist(families: typing.Mapping[str, prometheus_client.Metric],
                       metric_name: str,
                       labels: typing.Mapping[str, str],
                       value: float = None):
    metric = families[metric_name]

    samples = [s for s in metric.samples if
               all((label in s.labels and s.labels[label] == label_value) for label, label_value in labels.items())]
...
def text_string_to_metric_families_map(text) -> typing.Mapping[str, prometheus_client.Metric]:
    families: typing.Generator[prometheus_client.Metric] = parser.text_string_to_metric_families(text)
