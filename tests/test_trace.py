...
@pytest.mark.parametrize("namespace",
                         [None, "namespace"],
                         ids=["no_namespace", "custom_namespace"])
@pytest.mark.parametrize("registry_gen",
...
class TestTrace:
    ...
    @pytest.fixture
    def current_registry(self, registry: prometheus_client.CollectorRegistry) -> prometheus_client.CollectorRegistry:
        if registry:
            return registry
    ...
    async def client(self,
                     ...
                     client_name: Optional[str]) -> TestClient:
        params = {}
        if registry:
        ...
        return await aiohttp_client(app, trace_configs=[PrometheusTraceConfig(**params)],
    ...
    async def test_ok(self,
        ...
        current_frozen_registry: List[prometheus_client.Metric] = list(current_registry.collect())

        assert_metric_value(current_frozen_registry,
        ...
        assert_metric_exists(current_frozen_registry,
    ...
    async def test_parallel_connection(self,
                                       client: TestClient,
                                       client_name: str,
                                       namespace_prefix: str,
                                       current_registry: prometheus_client.CollectorRegistry):
    ...
    async def test_google(self,
        ...
        async with aiohttp.ClientSession(trace_configs=[PrometheusTraceConfig(**params)],
        ...
        async with aiohttp.ClientSession(trace_configs=[PrometheusTraceConfig(**params)]) as session:
            async with session.get('http://www.google.com/') as resp:
                assert resp.status
...
def get_metric_value(frozen_registry: List[prometheus_client.Metric],
                     metric_label: str,
                     sample_label: str,
                     labels: Dict[str, str]):
    for metric in frozen_registry:
        ...
        for sample in metric.samples:  # type: Sample
            if sample.name != sample_label:
                continue
            if all(label in sample.labels and label_value == sample.labels[label]
                   for label, label_value in labels.items()):
                return sample.value


def assert_metric_value(frozen_registry: List[prometheus_client.Metric],
                        metric_label: str,
                        sample_label: str,
                        expected_value: float,
                        labels: Dict[str, str]):
    value = get_metric_value(frozen_registry,
                             metric_label,
                             sample_label,
                             labels)
...
def assert_metric_exists(frozen_registry: List[prometheus_client.Metric],
                         metric_label: str,
                         sample_label: str,
                         labels: Dict[str, str]):
    value = get_metric_value(frozen_registry,
